	...
	*/
	public void createElementBoard()
	{
		...
		for(int i=0; i<snakes.length; i++) {
			...
			int headJ = (n-1-i)%2 == 0 ? (headId-1)%m : (m-1)-(headId-1)%m;
			int tailI = (n-1)-(tailId-1)/m;
			int tailJ = (n-1-i)%2 == 0 ? (tailId-1)%m : (m-1)-(tailId-1)%m;
		...
		for(int i=0; i<ladders.length; i++) {
			...
			int upStepJ = ((n-1-i)%2 == 0) ? (upStepId-1)%m : (m-1)-(upStepId-1)%m;
			int downStepI = (n-1)-(downStepId-1)/m;
			int downStepJ = ((n-1-i)%2 == 0) ? (downStepId-1)%m : (m-1)-(downStepId-1)%m;
		...
		for(int i=0; i<apples.length; i++) {
			...
			int tileJ = (n-1-i)%2 == 0 ? (tileId-1)%m : (m-1)-(tileId-1)%m;
