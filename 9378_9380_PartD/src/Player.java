...
public class Player {
	...
	//Constructor με όρισμα αντικείμενο
	...
	int[] move(int id, int die) {
		int[] array = new int[5]; // Array Containing {CurrentPosition, Snakes, Ladders, RedApples, BlackApples}
		id += die;
		...
		do {
			...
			for(int i=0; i<board.getApples().length; i++)
				if (board.getApples()[i].getAppleTileId() == id) {
					...
					board.getApples()[i].setAppleTileId(-1);
					
					if(board.getApples()[i].getColor() == "Red")
						array[3]++; // Number of Red Apples
					else if(board.getApples()[i].getColor() == "Black")
						array[4]++; // Number of Black Apples
					break;
				}
			for(int i=0; i< board.getSnakes().length; i++)
				if (board.getSnakes()[i].getHeadId() == id) {
					id = board.getSnakes()[i].getTailId();
					array[1]++; // Number of Snakes
					break;
			}
			for(int i=0; i< board.getLadders().length; i++)
				if (board.getLadders()[i].getUpStepId() == id) {
					if(board.getLadders()[i].getBroken() == false){
						id = board.getLadders()[i].getDownStepId();
						array[2]++;
						board.getLadders()[i].setBroken(true);
						//board.ladders[i].setUpStepId(-1);
						//board.getLadders()[i].setUpStepId(-1);
						break;
				}
			}
		}while(start != id);
	 ...
	 */
	protected Apple appleExists(int id) {
		for(int i=0; i<board.getApples().length; i++)
			if (board.getApples()[i].getAppleTileId() == id)
	...
	*/
	protected Snake snakeExists(int id) {
		for(int i=0; i< board.getSnakes().length; i++)
			if (board.getSnakes()[i].getHeadId() == id)
	...
	*/
	protected Ladder ladderExists(int id) {
		for(int i=0; i< board.getLadders().length; i++)
